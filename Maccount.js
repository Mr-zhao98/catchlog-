...
//  //  sleep(2000);
   ...
   // FuncConfig.put("账号位置","1")
   // FuncList["开始帐号"] = FuncConfig.get("开始帐号");
   // FuncList["开始序号"] = FuncConfig.get("开始序号");
    ...
    sleep(2000)
    while (!((text("我的").exists())||(text("登录").exists()))) {
        toastLog("正在检测主页");
       // sleep(2000);
        //back();
        sleep(1000);
    ...
    try {
        ...
        for (var l = 0; l < startNumber.length; l++){
            ...
            switch (startNumber[l]) {
                ...
                case 3:
                    toastLog("3");
                    if ((myScores['四人赛'] == 0)&&(FuncList['四人赛'])) {
                        competition(); //争上游答题
                    }
                    break;
                case 4:
...
//主函数
function main(){
    var getpass=open("./传参.txt","r");
    var 账号位置=getpass.readline();
    getpass.close();
   
    start_app(); //启动app
    path="/sdcard/用户数据.txt";
    //以读取模式打开文件
    file = open(path, "r")
    var usearr=file.readlines()
    file.close()
    //FuncConfig.put("账号位置","2")
    files.create("./读取进程.txt");
    if(账号位置>usearr.length){
        console.info("账号开始位置超出总账号个数，请输入正确的开始位置")
        return;
    }
        //for(i=(账号位置-1);i<usearr.length;i++){   
            //读取进程
        var next=true;
        var i=(账号位置-1);
        while(next){
          // FuncConfig = storages.create("强国学习配置"); 
            ...
            arrchild=usearr[i]
            var usepsw=arrchild.split("---");
            var 账号=usepsw[0]
            var 密码=usepsw[1]
          //  toastLog("当前账号密码为")
            log('当前账号:'+账号)
            log('当前密码:'+密码)
            log('总共:'+(usearr.length)+'个账号')
            var remain=usearr.length-i
            log('即将学习到第'+(usearr.length-remain+1)+'个账号')
            var 当前位置=usearr.length-remain+1;
              //以读写模式打开文件
            file = open("./读取进程.txt", "w")
            file.write(当前位置);
            file.close();
          //  FuncConfig.put("账号位置",账号位置)
          // log(FuncConfig.put("账号位置",账号位置))
           // log('剩余'+(usearr.length-i)+'个账号未学习')
            sleep(1000);
            登录(账号,密码);
            i++;
            //切换账号运行
...
/* 多账号登录函数*/
 function 登录(账号,密码){
           ...
           // click("我的");
            id("comm_head_xuexi_mine").findOne().click();
            sleep(800)
            while(!id("cn.xuexi.android:id/my_setting").exists());
            id("cn.xuexi.android:id/my_setting").findOne().click();
            sleep(500)
            while(!text("退出登录").exists());
            text("退出登录").findOne().parent().parent().parent().click();
            sleep(500)
            ...
            id("et_phone_input").findOne().setText(账号);
            sleep(500)
            //toastLog(密码)
            id("cn.xuexi.android:id/et_pwd_login").findOne().setText(密码);
            sleep(1000)
            //while(id("cn.xuexi.android:id/btn_next").clickable())
            id("cn.xuexi.android:id/btn_next").findOne().click();
            sleep(2000)
          // return 账号,密码;
          // return 密码;
